...
def create_psf2_file(font_data, psf2_file_path):
    ...
    height = 16
    width = 8
    glyph_size = int((height*width)/8)

    psf2_header = struct.pack('<4sIIIIIII', magic_bytes, version, header_size, flags, num_glyphs, glyph_size, height, width)

    glyph_data = b''.join([struct.pack('<16B', *row) for row in font_data])

    psf2_file_content = psf2_header + glyph_data

    with open(psf2_file_path, 'wb') as psf2_file:
...
if __name__ == "__main__":
    ...
    create_psf2_file(font_data, psf2_file_path)
    print(f"PSF2 file '{psf2_file_path}' created successfully.")
